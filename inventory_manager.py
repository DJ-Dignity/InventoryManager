...
"""
This module currently currently does the following tasks:
    - Parses through a JSON settings file for information on file management and MongoDB database.
    - Finds PDF's of order invoices and creates CSV's with the ordered parts' information.
    - Check each invoice to see if it's in MongoDB; add it if not.
Things that still need to be implemented:
    - GUI (probably with PyQT)
    - Auto-assign part numbers
    - Pick and choose which invoices to be added/updated to MongoDB (through GUI)
    - Parse and analyze BOM's generated by KiCad
"""
...
class OrderedPart:
    """This class describes an inventory part. The information is gathered and stored in a CSV and later into MongoDB"""
    def __init__(self):
...
def parse_settings_json():
    """Parse through settings.json file and save settings into appropriate list/dict"""
...
def create_csvs():
    """Parse through invoice PDF and create a CSV with ordered parts' information."""
            ...
            if dir_name.endswith(supplier):
                for filename in file_names:
                    ...
                    if file_name.endswith(".pdf"):
                        ...
                        match supplier:
                            case "Digikey":
                                #invoice_no, order_no, order_date = Digikey_order_parsing(os.path.join(dir_name, file_name))
                                pass
                            case "Tayda":
                                order_date, order_no, ordered_parts = Tayda_order_parsing(os.path.join(dir_name, file_name))
                                if file_name_split[0] + ".csv" not in file_names:
                                    create_order_csv(file_name, ordered_parts)
                            case "Small Bear":
                                order_date, order_no, ordered_parts = Small_Bear_order_parsing(os.path.join(dir_name, file_name))
                                if file_name_split[0] + ".csv" not in file_names:
                                    create_order_csv(file_name, ordered_parts)
                            case other:
                                ...
                                break
                        if supplier != "Digikey":
                            print("Created CSV for %s order #%s" % (supplier, order_no))
                    else:
                        print("pdf found with csv conversion: %s" % os.path.join(dir_name, file_name))


def Tayda_order_parsing(file_name, parse_parts=True):
    """Parse through a Tayda invoice PDF and return order number, date, and ordered parts."""
    ordered_parts = []
    ...
    #print("page_strings:")
    #print(page_strings)
    for page_idx, page in enumerate(page_strings):
        if page_idx == 0:
        ...
        for string in page:
            ...
            match search_parameter:
                case "date":
                    ...
                    order_date = string.split(",")[0]
                    search_parameter = "order_no"
                case "order_no":
                    # string = Order # <order_no>
                    order_no = string.split("Order # ")[1]
                    if parse_parts:
                        break
                    else:
                        search_parameter = "start_of_table"
    ...
    if parse_parts:
        return order_no, order_date, ordered_parts
    else:
        return order_no, order_date


def Small_Bear_order_parsing(file_name, parse_parts=True):
    """Parse through a Small Bear invoice PDF and return order number, date, and ordered parts."""
    ordered_parts = []
    # Open PDF and parse the strings in it
    page_strings = parse_pdf_strings(file_name)
    idx = 0
        ...
        for string in page:
            ...
            match search_parameter:
                case "date":
                    ...
                    search_parameter = "order_no"
                case "order_no":
                    if string.startswith("Small Bear Electronics Invoice"):
                        order_no = string.split("#")[1]
                        if parse_parts:
                            break
                        else:
                            search_parameter = "start_of_table"
...
def Digikey_order_parsing(file_name):
    """Parse through a Digikey invoice PDF and return order number, invoice number, and date."""
    # Open PDF and parse the strings in it
    page_strings = parse_pdf_strings(file_name)
    idx = 0
    order_date = ""
...
def parse_csv(file_name):
    """Parse through a CSV file and return a list of ordered parts."""
    is_error = False
    ordered_parts = []
    """Parse part details from CSV file"""
    with open(file_name, newline="") as csvfile:
        reader = csv.reader(csvfile, delimiter=",")
        for idx, row in enumerate(reader):
            part = {}
            # Check that the header matches the expected strings
            for category, row_value in zip(csv_fields, row):
                if idx == 0:
                    if len(csv_fields) != len(row):
                        print("Error when parsing CSV: Incorrect number of categories.")
                        is_error = True
                        break
                    if category != row_value:
                        print("Error when parsing CSV: Category does not match expected value.")
                        is_error = True
                        break
                else:
                    part[category] = row_value
            if idx != 0:
...
def create_order_csv(file_name, ordered_parts):
    ...
    with open(csv_name, "w", newline="") as file:
        ...
        for part in ordered_parts:
            ...
            #print("printing part: %s" % part.part_description)
    print("Created CSV %s" % csv_name)


def assign_part_number(ordered_parts):
    """Parse through a list of ordered parts and assign a part number to each based on the part description."""
...
def update_db_orders():
    """Log in to MongoDB and update the order DB with ordered not yet present in it."""
    ...
    for dir_name, dir_names, file_names in os.walk(order_settings["order_wd"]):
        for supplier in suppliers_list:
            if dir_name.endswith(supplier):
                for short_file_name in file_names:
                    if short_file_name.endswith(".pdf"):
                        file_name = os.path.join(dir_name, short_file_name)
                        if supplier == "Digikey":
                            invoice_no, order_no, order_date = Digikey_order_parsing(file_name)
                            file_name_csv = file_name.replace(invoice_no, order_no).replace(".pdf", ".csv")
                            #print("file_name_csv = %s" % file_name_csv)
                        elif supplier == "Small Bear":
                            file_name_csv = file_name.replace(".pdf", ".csv")
                            order_no, order_date = Small_Bear_order_parsing(file_name, parse_parts=False)
                        elif supplier == "Tayda":
                            file_name_csv = file_name.replace(".pdf", ".csv")
                            order_no, order_date = Tayda_order_parsing(file_name, parse_parts=False)
                        else:
                            print("Supplier not found. Cannot parse or upload information from %s." % short_file_name)
                            break
                        ordered_parts = parse_csv(file_name_csv)
                        doc_exists = orders_coll.count_documents({"order_no": int(order_no)})
                        if not doc_exists:
                            if supplier == "Digikey":
                                orders_coll.insert_one({"order_no": int(order_no), "invoice_no": int(invoice_no), "order_date": order_date, "ordered_parts": ordered_parts})
                            else:
                                orders_coll.insert_one({"order_no": int(order_no), "order_date": order_date, "ordered_parts": ordered_parts})
                            print("Created MongoDB entry for %s order #%s" % (supplier, order_no))
